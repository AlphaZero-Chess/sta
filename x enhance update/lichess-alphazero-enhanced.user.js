...
(function() {
    ...
    class MoveGenerator {
        static generate(board, onlyLegal = true) {
            ...
            // CRITICAL: Filter out illegal moves that leave king in check
            if (onlyLegal) {
        ...
        static async query(fen) {
            ...
            try {
                ...
                return new Promise((resolve) => {
                    // Try with GM_xmlhttpRequest if available
                        ...
                        GM_xmlhttpRequest({
                            ...
                            onload: function(response) {
                                try {
                                    ...
                                    resolve(TablebaseClient.parseResponse(data));
                                } catch (e) {
                                    Logger.debug(`Tablebase parse error: ${e.message}`);
                                    resolve(null);
                                }
                            },
                            onerror: function() {
    ...
    class ChessEngine {
        ...
        async getBestMove(fen, timeLimit = 2000) {
            ...
            Logger.info(`Analyzing position: ${board.toFEN()}`);
            ...
            if (this.useTablebase && board.getPieceCount() <= 5) {
                ...
                if (tbResult && tbResult.move) {
                    Logger.tablebase(`Perfect move found: ${tbResult.move} (DTZ: ${tbResult.dtz}, WDL: ${tbResult.wdl})`);
            ...
            if (this.useMCTS) {
                ...
                // Use MCTS for positions with many pieces or complex middlegame
                if (board.getPieceCount() > 10 || moves.length > 30) {
                    Logger.mcts('Using MCTS (complex position)');
                    const move = this.mcts.search(board, MCTS_CONFIG.SIMULATIONS);
                    if (move) return MoveGenerator.moveToUCI(move);
                }
            }
            
            // Strategy 3: Use Alpha-Beta for tactical positions
            ...
            const moves = MoveGenerator.generate(board, true);
            if (moves.length > 0) {
                Logger.warn('Using fallback random move');
                return MoveGenerator.moveToUCI(moves[0]);
            }
